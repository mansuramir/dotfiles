...
--capslock.widget.font = 
--capslock.font_color = "#d4be98"

-- markup(theme.fg_normal, "%d %b ") .. markup(theme.fg_normal, "%-I:%M%p"))

--capslock.activated = markup(self.font, "<b>A</b>"))
capslock.activated = "<b>CAPS ON</b>"
capslock.deactivated = "<b>caps off</b>"
...
function capslock:check()
  --self.widget.font = "CaskaydiaCove Nerd Font bold 8"
    ...
    "bash -c 'sleep 0.2 && xset q'",
    {
      stdout = function (line)
        if line:match("Caps Lock") then
          ...
          tooltip.markup = "Caps Lock " .. status
          if status == "on" then
            self.markup = self.activated
            --self.markup = markup("CaskaydiaCove Nerd Font bold 8", "A"))
          else
